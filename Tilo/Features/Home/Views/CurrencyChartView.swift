...
final class CurrencyChartViewModel: ObservableObject {
    ...
    func fetchRates(for range: TimeRange = .oneWeek) async {
        ...
        debugLog("📊 ViewModel fetchRates: \(fromCurrency) → \(toCurrency)")
        ...
        if let currentRate = await exchangeService.getRate(from: fromCurrency, to: toCurrency) {
            ...
            debugLog("📊 Showing estimated data instantly (current rate: \(currentRate))")
        }
        ...
        if let historicalData = await exchangeService.fetchHistoricalRates(from: fromCurrency, to: toCurrency, days: 14) {
            debugLog("📊 Received \(historicalData.count) historical data points")
            ...
            debugLog("📊 Chart updated with real data: \(rates.count) points")
        ...
        isLoading = false
        debugLog("📊 fetchRates completed, isLoading: \(isLoading), rates count: \(rates.count)")
    }
...
struct CurrencyChartView: View {
    ...
    init(fromCurrency: String, toCurrency: String) {
        debugLog("🔄 CurrencyChartView init: \(fromCurrency) → \(toCurrency)")
    ...
    var body: some View {
        ...
        .onChange(of: toCurrencyCode) { oldValue, newValue in
            debugLog("📊 toCurrencyCode changed: \(oldValue) → \(newValue)")
